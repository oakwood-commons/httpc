...
func TestMemoryCacheWrapper_SetAndGet(t *testing.T) {
	...
	cache := newMetricsMemoryCache(baseCache, NoopMetrics{})
...
func TestMemoryCacheWrapper_EmptyDataHandling(t *testing.T) {
	...
	// Test with empty byte slice - empty data is a valid cached entry
	// (e.g. HTTP 204 No Content responses)
	err := cache.Set(ctx, "empty-key", []byte{}, 1*time.Minute)
	require.NoError(t, err)

	// Get should treat empty data as a cache hit (not a miss)
	data, err := cache.Get(ctx, "empty-key")
	assert.NoError(t, err)
	assert.Equal(t, []byte{}, data)

	// Check stats - should be a hit
	hits, misses := cache.Stats()
	assert.Equal(t, uint64(1), hits)
	assert.Equal(t, uint64(0), misses)
}
